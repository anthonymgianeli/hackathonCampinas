...
struct Network {
    ...
    func request<T: AbstractRequest, U : AbstractResponse>(request : T, completion : @escaping (U)-> ()) {
        ...
        // Request
        var urlRequest = URLRequest(url: APIConfig.ServiceURL.MountURL(path: request.path))
        ...
        if (request.httpMethod == .post || request.httpMethod == .put) {
            ...
            if let params = request.toDictionary() as? [String:String] {
                
                let url = APIConfig.ServiceURL.MountURL(path: request.path, params: params)
                urlRequest.url = url
            }
